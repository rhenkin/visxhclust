...
The clustering itself takes three steps: computing a distance matrix, computing the hierarchical clusters and cutting the tree to find the desired number of clusters. In the app, each of these steps has matching parameters: choice of scaling and distance/similarity metric, linkage method and the number of clusters. 

```{r params}
scaling_method <- "z-scores"
...
```{r computation}
dmat <- compute_dmat(subset_data, distance_method, scaling_method)
...
cluster_heatmaps(scale_data(subset_data), 
                 clusters,
                 k,
                 cluster_colors,
